"""
Módulo de análisis de bifurcaciones 1D
...
class BifurcationAnalyzer:
    ...
    def __init__(self, function_str: str):
        ...
        # Parsear la función con símbolos locales
        try:
            # Crear diccionario de símbolos locales para que sympify use nuestros símbolos
        ...
        # Calcular derivada respecto a x (para estabilidad)
        self.df_dx = sp.diff(self.f, self.x)
        
    def find_equilibria(self, r_value: float = None) -> List[sp.Expr]:
        """
        ...
        if r_value is not None:
            # Convertir a racional para evitar problemas de rendimiento con floats
            r_rational = sp.Rational(r_value).limit_denominator(1000)
            eq = self.f.subs(self.r, r_rational)
        else:
            eq = self.f
            
        # Resolver sin simplificación para mejorar rendimiento
        equilibria = sp.solve(eq, self.x, rational=False, simplify=False)
        return equilibria
    
    def stability(self, x_eq: float, r_value: float) -> str:
        """
        ...
        Returns:
            'stable' si df/dx < 0, 'unstable' si df/dx > 0, 'neutral' si df/dx = 0
        """
    ...
    def get_equilibria_with_stability(self, r_value: float) -> List[Dict]:
        """
        ...
        Returns:
            Lista de diccionarios con 'x' y 'stability'
        """
        equilibria = self.find_equilibria(r_value)
        results = []
        ...
        for eq in equilibria:
            try:
                ...
                x_val = complex(eq)
                if abs(x_val.imag) < 1e-10:  # Solo puntos reales
                    x_float = float(x_val.real)
                    stab = self.stability(x_float, r_value)
                    results.append({'x': x_float, 'stability': stab})
            except (TypeError, ValueError):
                # Solución simbólica que no se puede evaluar
                pass
                
        return results
    
    def generate_bifurcation_data(self, r_range: Tuple[float, float], 
        ...
        for r_val in r_values:
            eq_data = self.get_equilibria_with_stability(r_val)
        ...
        return {
            'stable': {'r': np.array(stable_r), 'x': np.array(stable_x)},
            'unstable': {'r': np.array(unstable_r), 'x': np.array(unstable_x)}
        }
    
    def evaluate_function(self, x_vals: np.ndarray, r_value: float) -> np.ndarray:
        """
